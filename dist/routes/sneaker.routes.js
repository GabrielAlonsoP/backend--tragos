...
tragoRoutes.get('/pagin', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let skip = page - 1;
    skip = skip * perPage;
    const trago = yield sneaker_model_1.Trago.find().skip(skip).limit(perPage);
    return res.json({
        ok: true,
        msj: "Get OK",
        trago
...
tragoRoutes.get('/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const sneaker = yield sneaker_model_1.Trago.find();
    return res.json({
        ok: true,
        msj: "Get OK",
        Trago: sneaker_model_1.Trago
    });
}));
tragoRoutes.post('/', (req, res) => {
    ...
    sneaker_model_1.Trago.create(trago).then(tragoDb => {
        console.log(tragoDb);
        return res.json({
            ok: true,
...
tragoRoutes.put('/:id', (req, res) => {
    ...
    sneaker_model_1.Trago.findByIdAndUpdate(tragoId, trago).then(tragoDb => {
        return res.json({
            ok: true,
            tragoDb
        });
...
tragoRoutes.delete('/', (req, res) => {
    ...
    sneaker_model_1.Trago.findByIdAndDelete(tragoId).then(tragoDb => {
        return res.json({
            ok: true,
            tragoDb
        });
